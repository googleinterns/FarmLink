  ...
  return new Promise(function (resolve, reject) {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (
          parseInt(distanceText.split(" ")[0].replace(/,/g, "")) <
          parseInt(request.query.distance)
        ) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (
          convertTime(days, hours, minutes) <
          convertTime(
            parseInt(request.query.days),
            parseInt(request.query.hours),
            parseInt(request.query.minutes)
          )
        ) {
...
exports.queryFoodBanksByDistance = (request, response) => {
  db.collection("foodbanks")
    ...
    .then((data) => {
      ...
      Promise.all(
        foodbanks.map(function (foodbank) {
  ...
  db.collection("surplus")
    ...
    .then((data) => {
      ...
      function queryFarms(data) {
        return new Promise(function (resolve, reject) {
          db.doc(`/farms/${data.originFarmId}`)
            .get()
            .then((doc) => {
              let surplus = data;
              let farm = doc.data();
